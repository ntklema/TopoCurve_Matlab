...
for i=3
    ...
    if i==1
        in=find(and(and(dist>2500,dist<=11000),~isnan(S2.Z)));
        bc=bin(Out_S.trench_y(in),Out_S.trench_x(in),nb);
        b=bin(Out_S.trench_y(in),S2.Z(in),nb);
        [lat, ~] = projinv(projcrs(26910), bc(:,2)', bc(:,1)');
        ...
        nin=find(s2~=0);
        s2=smoothdata(s2(nin),'gaussian',5);

        % fill([lat(nin) fliplr(lat(nin))],[g_min(nin) fliplr(g_max(nin))],cmap(i,:),'FaceAlpha',0.8); hold on
        plot(lat(nin),s2,'k','LineWidth',1); 


    else

        in=find(and(and(dist>d(i),dist<=d(i+1)),~isnan(S2.Z)));
